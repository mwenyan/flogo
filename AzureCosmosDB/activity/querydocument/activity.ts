...
@WiContrib({})
@Injectable()
export class QueryDocumentActivityContributionHandler extends WiServiceHandlerContribution {
    ...
    value = (fieldName: string, context: IActivityContribution): any | Observable<any> => {
        ...
        let query = context.getField("query").value;
        let queryParams = context.getField("queryParams").value;
        switch(fieldName) {
            case "connector":
                let connections = [];
                ...
                if(Boolean(query)){
                    if(Boolean(queryParams)){
                        if(queryParams.value) {
                            let inputs = JSON.parse(queryParams.value);
                            ...
                            for(let i=0; i<inputs.length; i++){
                                 ...
                                 if(repeating)
                                    schema.properties["parameters"]["properties"][name] = {type: "array", items: {type: tp}}
                                 else 
                                    schema.properties["parameters"]["properties"][name] = {type: tp}
                            }
                            schema.required = ["database", "collection", "parameters"]
                        }
                    }
                }
