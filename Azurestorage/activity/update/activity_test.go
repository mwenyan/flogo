...
func Test_Create_file_ConflictReplace(t *testing.T) {
	...
	tc.SetInput("operation", "Write Content")
...
func Test_clearFile(t *testing.T) {
	log.RootLogger().Info("****TEST : Executing InsertOne start****")
	m := make(map[string]interface{})
	err1 := json.Unmarshal([]byte(settingsjson), &m)
	assert.Nil(t, err1)
	...
	tc.SetInput("Connection", m)
	tc.SetInput("service", "File")
	tc.SetInput("operation", "Clear Content")
	var inputIntf interface{}
	err2 := json.Unmarshal([]byte(clearFile), &inputIntf)
...
func Test_Create_dir_fail(t *testing.T) {
	...
	tc.SetInput("Connection", m)
	tc.SetInput("service", "File")
	tc.SetInput("operation", "Create Directory")
	var inputIntf interface{}
	err2 := json.Unmarshal([]byte(inputSchemaDirFail), &inputIntf)
